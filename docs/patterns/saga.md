...
| Aspect | Choreography | Orchestrated-async |
|--------|--------------|--------------------|
| Control flow | Distributed — each service reacts and emits the next | Centralized in one owner (the saga definition) |
| Visibility | No single record — reconstruct from each service's state | One `saga_instance` row: status, current step, deadline, retries |
| Coupling | Loosest — services share only event contracts | Owner knows step order; participants still only see events |
| Compensation | Each service choreographs its own undo | Engine drives `compensate()` in reverse; timeout-driven |
| Best for | Simple, linear, 2–3 hop flows | Multi-step flows needing a central view / deadlines / branching |
| In this repo | plain `POST /api/order` ([choreographed-stock-reservation.md](choreographed-stock-reservation.md)) | `POST /api/order/saga` — reserve-stock is an async step |
